...
def get_label(x1, x2):
    ...
    return np.logical_and(g<=-1., x2>0)*2-1
...
if __name__ == "__main__":
    ...
    L_test = get_label(D_test[:,0], D_test[:,1])
    ...
    while i < n_iter+1:
        ...
        pool, bounds_new = expand_pool(D, bounds_old, expansion_rate=1.*length_scale)
        for entry in pool:
...
#        clf.train(dataset)
        ...
        if i%interval == 0:
            ...
            Z3 = get_label(grid[:,0], grid[:,1]) # to show ground truth decision boundary
            Z3 = Z3.reshape(xx.shape)
            
            ax = plt.subplot(im_rows, im_cols, i/interval+1)
#            image = plt.imshow(Z0, interpolation='nearest',
#                               extent=(xx.min(), xx.max(), yy.min(), yy.max()),
#                               aspect='auto', origin='lower', cmap=plt.cm.PuOr_r)
#            plt.colorbar(image)
            plt.contour(xx, yy, Z1, levels=[0], linewidths=2, linetypes='--', c='g', alpha=0.5) # estimated decision boundary
            plt.contour(xx, yy, Z4, levels=[0.8*norm.cdf(-margin)], linewidths=1, linetypes=':', c='b', alpha=0.3)
            plt.imshow(Z3<0, interpolation='nearest',
                       extent=(xx.min(), xx.max(), yy.min(), yy.max()),
