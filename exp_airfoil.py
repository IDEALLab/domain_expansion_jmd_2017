"""
Active Domain Expansion for the airfoil example
...
from synthesis import synthesize_shape
...
def get_label(X):
    ...
    m = X.shape[1]
    # The upper curve doesn't intersect with the lower curve
    ind1 = True
    ind2 = True
    for i in range(1,m/2+1,2):
        ind1 = np.logical_and(X[:,i]>X[:,m-i], ind1)
        ind2 = np.logical_and(X[:,i]<X[:,m-i], ind2)
    
    # The aspect ratio is lower than a threshold
    h = np.max(X[:,1:m:2], axis=1) - np.min(X[:,1:m:2], axis=1)
    w = np.max(X[:,0:m-1:2], axis=1) - np.min(X[:,0:m-1:2], axis=1)
    ind3 = h/w < .4
    
    L = np.logical_and(np.logical_or(ind1, ind2), ind3)
    L = L*2-1
    return L
    
def gen_samples(d, bounds, density=250):
...
if __name__ == "__main__":
    ...
    D = D0
    X = synthesize_shape(D, c=c, model_name=model_name, raw=False)
    L = get_label(X)
    print L
    dataset = Dataset(D, L)
    
    # Generate test set
    D_test = gen_samples(d, BD, density=50)
    X_test = synthesize_shape(D_test, c=c, model_name=model_name, raw=False)
    L_test = get_label(X_test)
...
#        clf.train(dataset)
        ...
        if i%interval == 0:
            ...
            X_grid = synthesize_shape(grid, c=c, model_name=model_name, raw=False)
            Z3 = get_label(X_grid) # to show ground truth decision boundary
...
#            plt.title("Iteration: %d" % i, fontsize=20)
        ...
        x = synthesize_shape(new, c=c, model_name=model_name, raw=False)
        l = get_label(x)
