...
def stsc(X, w, C, eigen_gap=0, verbose=0):
    ...
    X: sorted eigenvectors of Laplacian matrix
...
def rmmsl(X, sigma_n=1, sigma_e=1, sigma_c=1, verbose=0):
...
#                w2 = math.exp(-np.inner(theta, theta)*b**2/d_sq/sigma_c**2) # curved level kernel
                W[i,j] = w1 * w2
